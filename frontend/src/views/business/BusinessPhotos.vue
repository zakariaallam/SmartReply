<template>
  <div class="p-8 max-w-6xl mx-auto">
    ...
    <div v-else-if="images.length === 0"
      class="bg-white border-2 border-dashed border-gray-200 rounded-3xl p-16 text-center">
      ...
      <p class="text-gray-500 mb-8 max-w-sm mx-auto">Les entreprises avec des photos reçoivent jusqu'à 3 fois plus de
        réservations.</p>
      <button @click="showUploadModal = true"
        class="px-8 py-3 bg-indigo-600 text-white rounded-full font-bold hover:bg-indigo-700 transition-colors">
        Commencer à ajouter
      </button>
    </div>
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      <div v-for="img in images" :key="img.id"
        class="group relative aspect-square bg-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow border border-gray-100">
        <img :src="img.image_url"
          class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" alt="Gallery">
        ...
        <div
          ...
          <button v-if="!img.is_cover" @click="setCover(img.id)"
            class="p-2 bg-white text-gray-900 rounded-full hover:bg-indigo-600 hover:text-white transition-colors"
          ...
          <button @click="deleteImage(img.id)"
            class="p-2 bg-white text-gray-900 rounded-full hover:bg-red-600 hover:text-white transition-colors"
            title="Supprimer">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div v-if="img.is_cover"
          class="absolute top-3 left-3 bg-white px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider text-indigo-600 shadow-sm">
          Couverture
        </div>
      </div>
    </div>
        ...
        <div class="p-8">
          ...
          <div class="space-y-4">
            <div>
              ...
              <input type="file" @change="handleNameImage"
...
const imageMinio = ref(null);

const handleNameImage = (e) => {
  const file = e.target.files?.[0];
  imageMinio.value = file;
...
const uploadImage = async () => {
  ...
  try {
    ...
    formData.append('image', imageMinio.value)

    const res = await api.postForm('/upload/image', formData);
