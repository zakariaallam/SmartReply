<template>
  <div class="w-full">
    ...
    <form class="space-y-5" @submit.prevent="ownerRegister">
      
       <!-- business -->
      <div class="flex flex-col text-left">
        <label class="font-bold text-[14px] mb-1">Business Name *</label>
        <input v-model="form.business_name" type="text" class="border border-gray-200 rounded-lg px-4 py-3 outline-none focus:border-black transition text-[15px]" />
      </div>
...
const ownerRegister = async () => {
  ...
  try {
    ...
    const res = await api.post('/ownerRegister', form)
    console.log(res)
